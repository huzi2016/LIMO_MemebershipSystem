<?php
/**
 * Plugin Name: My Custom WC Account Ultimate 14.0.0
...
class SMP_Member_ID {
    ...
    public static function init() {
        add_action( 'user_register',                       [ __CLASS__, 'assign'           ] );
        add_action( 'wp_login',                            [ __CLASS__, 'on_login'          ], 10, 2 );
        add_action( 'show_user_profile',                   [ __CLASS__, 'render_admin_field' ] );
        add_action( 'edit_user_profile',                   [ __CLASS__, 'render_admin_field' ] );
        add_action( 'woocommerce_edit_account_form_start', [ __CLASS__, 'render_account_cards' ] );
        add_filter( 'smp_dashboard_member_id',             [ __CLASS__, 'get_current'       ] );
        add_filter( 'manage_users_columns',                [ __CLASS__, 'add_columns'        ] );
        add_filter( 'manage_users_custom_column',          [ __CLASS__, 'render_column'      ], 10, 3 );
        add_filter( 'manage_users_sortable_columns',       [ __CLASS__, 'sortable_columns'   ] );
        add_action( 'pre_get_users',                       [ __CLASS__, 'handle_sort'        ] );
    }
...
class SMP_Admin_Points_Field {
    ...
    public static function init() {
        ...
        add_action( 'wp_ajax_smp_admin_adjust_points', [ __CLASS__, 'ajax_adjust_points' ] );
    }
    ...
    public static function ajax_adjust_points() {
        ...
        $msg         = '';
        if ( $action === 'add' ) {
            SMP_Points_Engine::add_user_points( $user_id, $amount, $note ?: 'Manual addition by admin' );
            $msg = $amount . ' points added';
        } elseif ( $action === 'deduct' ) {
            $amount = min( $amount, $current_pts );
            SMP_Points_Engine::deduct_user_points( $user_id, $amount );
            $msg = $amount . ' points deducted';
        } elseif ( $action === 'set' ) {
            $delta = $amount - $current_pts;
            if ( $delta > 0 )      SMP_Points_Engine::add_user_points( $user_id, $delta,       $note ?: 'Balance set by admin' );
            elseif ( $delta < 0 )  SMP_Points_Engine::deduct_user_points( $user_id, abs($delta) );
...
class SMP_Assets {
    ...
    public static function init() {
        ...
        add_action( 'wp_enqueue_scripts',    [ __CLASS__, 'enqueue_global_css'        ], 20 );
        add_action( 'admin_enqueue_scripts', [ __CLASS__, 'enqueue_admin'             ] );
        add_action( 'wp',                    [ __CLASS__, 'maybe_load_for_shortcodes' ] );
    }
...
class SMP_Order_Ajax {
    ...
    public static function init() {
        add_action( 'wp_ajax_smp_get_admin_details',  [ __CLASS__, 'ajax_get_admin_details' ] );
        add_action( 'wp_ajax_smp_ajax_update_status', [ __CLASS__, 'ajax_update_status'     ] );
    }
...
class SMP_Checkout_Points {
    ...
    public static function init() {
        add_action( 'woocommerce_before_checkout_form',   [ __CLASS__, 'render_panel_early'       ], 5 );
        add_action( 'wp_footer',                          [ __CLASS__, 'render_panel_late'        ], 5 );
        add_action( 'wp_ajax_smp_get_pts_limits',         [ __CLASS__, 'ajax_get_pts_limits'      ] );
        add_action( 'wp_ajax_smp_apply_pts_discount',     [ __CLASS__, 'ajax_apply_pts_discount'  ] );
        add_action( 'wp_ajax_smp_clear_pts_discount',     [ __CLASS__, 'ajax_clear_pts_discount'  ] );
        add_action( 'woocommerce_cart_calculate_fees',    [ __CLASS__, 'cart_calculate_fees'      ] );
...
class SMP_Shortcodes {
    ...
    public static function init() {
        add_shortcode( 'order_lookup', [ __CLASS__, 'order_lookup' ] );
        add_shortcode( 'smp_auth',     [ __CLASS__, 'smp_auth'     ] );
    }
...
class SMP_Admin_Pages {
    ...
    public static function init() {
        ...
        add_action( 'admin_init', [ __CLASS__, 'save_pts_settings'        ] );
        add_action( 'admin_init', [ __CLASS__, 'save_member_num_settings' ] );
        add_action( 'admin_init', [ __CLASS__, 'save_topup_settings'      ] );
        add_action( 'admin_init', [ __CLASS__, 'reset_pts_settings'       ] );
        add_filter( 'woocommerce_get_order_item_totals', [ __CLASS__, 'rename_fees_label' ], 10, 3 );
    }

    public static function register_menu() {
        add_menu_page( 'CustomerAnalytics', '📊 CustomerAnalytics', 'manage_options', 'smp-customer-analytics', [ __CLASS__, 'render_customers_page' ], 'dashicons-chart-bar', 71 );
        add_submenu_page( 'smp-customer-analytics', 'Customers',           '👥 Customers',   'manage_options', 'smp-customer-analytics', [ __CLASS__, 'render_customers_page'       ] );
        add_submenu_page( 'smp-customer-analytics', 'Membership Settings', '⭐ Memebership', 'manage_options', 'smp-points-settings',     [ __CLASS__, 'render_points_settings_page' ] );
    }
...
class SMP_Automation {
    ...
    public static function init() {
        add_action( 'smp_topup_dispatch_hook',        [ __CLASS__, 'dispatch_customer_recharges' ] );
        add_action( 'smp_monthly_main_dispatch_hook', [ __CLASS__, 'dispatch_customer_recharges' ] );
        add_action( 'smp_as_do_single_recharge',      [ __CLASS__, 'execute_recharge_logic'      ] );
        add_action( 'wp_ajax_smp_topup_run_now',      [ __CLASS__, 'ajax_topup_run_now'          ] );
        add_filter( 'action_scheduler_retention_period', static function() { return DAY_IN_SECONDS * 30; } );
    }
    ...
    public static function get_config() {
        ...
        if ( ! is_array( self::$config_cache['roles'] ) ) self::$config_cache['roles'] = SMP_RECHARGE_ROLES;
        return self::$config_cache;
    }
    ...
    public static function sync_schedule( $config = null ) {
        ...
        if ( $config === null ) $config = self::get_config();
        as_unschedule_all_actions( 'smp_topup_dispatch_hook' );
        as_unschedule_all_actions( 'smp_monthly_main_dispatch_hook' );
        if ( empty( $config['enabled'] ) ) return;
        if ( function_exists( 'as_schedule_single_action' ) ) as_schedule_single_action( self::next_run_timestamp(), 'smp_topup_dispatch_hook' );
    }
    ...
    public static function ajax_topup_run_now() {
        ...
        if ( function_exists( 'as_enqueue_async_action' ) ) {
            ...
            foreach ( $user_ids as $user_id ) { self::execute_recharge_logic( (int) $user_id ); $count++; }
...
class SMP_Login_Redirect {
    ...
    public static function init() {
        add_action( 'init',         [ __CLASS__, 'redirect_wp_login'  ] );
        add_filter( 'login_url',    [ __CLASS__, 'filter_login_url'    ], 100, 2 );
        add_filter( 'register_url', [ __CLASS__, 'filter_register_url' ], 100 );
    }
...
register_activation_hook(   __FILE__, [ 'SMP_Automation', 'on_activation'   ] );
register_deactivation_hook( __FILE__, [ 'SMP_Automation', 'on_deactivation' ] );
